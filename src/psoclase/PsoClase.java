...
//        Algoritmo a = new Algoritmo(dimensions, Algoritmo.MAX, "6x-(x^2)");
        ...
        a.findPBest(); //comineza buscando pbest =>mejor punto global  y pBestLocation =>mejor punto personal

        int t = 0; //contador de generacion o iteracion

        double err = 9999; //error de parada 
        // ejemplo cuando una particula se aleja del enjambre 

        while (t < MAX_ITERATION ) {  //condicion de parada 
            //determina el mejor punto global y local
        ...
        /////// fin del algoritmo
        System.out.println("\nLa solicion encontrada de generacion " + (t - 1) + " , la solucion es:");
        for (int d = 0; d < a.dimensions.size(); d++) {
            System.out.println("     Mejor " + a.dimensions.get(d).name + ": " + a.gBestLocation.getLoc()[d]);
        }
